...
return ()=> clearTimeout(debounceFn);
  ...
  const getData= async ()=>{
    ...
    let res= await axios.get(`https://herofit-app-server.onrender.com/cardio?q=${search}`)
...
useEffect(() => {
  ...
  const fetchProductData = async () => {
    let url=`https://herofit-app-server.onrender.com/cardio?_page=${page}&_limit=9`
    // if(sort==="high"){
...
<select name="" id="" onChange={(e)=>{setSort(e.target.value)}}>
      ...
      {
      state.map((e)=>(
        <div key={e.id}    onClick={()=>navigate(`singlecardio/${e.id}`)}
        >
          <h3> {e.title}</h3>
    ...
    <div>
      ...
      {
        product?.map((e)=>(
          <div  style={{display: "grid" ,boxShadow: "rgba(0, 0, 0, 0.16) 0px 1px 4px, rgb(51, 51, 51) 0px 0px 0px 3px" }} onClick={()=>navigate(`singlecardio/${e.id}`)}>
      ...
      <button onClick={()=>navigate(`singlecardio/${e.id}`)}>More Details</button>
        </div>
