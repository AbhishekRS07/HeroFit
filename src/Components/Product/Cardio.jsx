...
const Cardio = () => {
  ...
  const fetchProductData = async () => {
    let url = `http://localhost:8080/cardio?page=${page}&limit=9`;
    if (filter) {
      url += `&category=${filter}`;
    }
    try {
  ...
  const getData = async () => {
    let res = await axios.get(`http://localhost:8080/cardio?q=${search}`);
    ...
    <div id="proddiv">
      ...
      <div id="options">
        ...
        <ul style={{ color: "white" }}>
          {state.map((e) => (
            <div key={e.id} onClick={() => navigate(`/cardio/${e.id}`)}>
              <h3> {e.title}</h3>
            </div>
          ))}
        </ul>
          ...
          {product?.map((e) => (
            ...
            >
              ...
              <div id="maintext">
                ...
                <button onClick={() => navigate(`/cardio/${e.id}`)}>
                  More Details
                </button>
              </div>
            </div>
